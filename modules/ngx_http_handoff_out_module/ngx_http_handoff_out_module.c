...
//static char *ifname = "ens1f0np0";
//static char *ifname = "enp23s0f0np0";
char *ifname = "enp8s0f0np0";
...
struct sockaddr_in peer_sockaddr[4];
int num_peers = 0;
...
{
    ngx_http_handoff_out_loc_conf_t  *conf;

    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_handoff_out_loc_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
#include "util.h"
    ...
    memcpy(&my_sockaddr, (struct sockaddr_in *)&ifr.ifr_addr, sizeof(struct sockaddr_in));
...
//192.168.11.53	n35
    ...
    num_peers = 0;
    // n31 - backend
    memset(&peer_sockaddr[num_peers], 0, sizeof(peer_sockaddr[num_peers]));
    inet_pton(AF_INET, "192.168.11.11", &peer_sockaddr[num_peers].sin_addr);
    peer_sockaddr[num_peers].sin_port = htons(79);
    peer_sockaddr[num_peers].sin_family = AF_INET;
    num_peers++;

    // n33 - backend
    memset(&peer_sockaddr[num_peers], 0, sizeof(peer_sockaddr[num_peers]));
    inet_pton(AF_INET, "192.168.11.33", &peer_sockaddr[num_peers].sin_addr);
    peer_sockaddr[num_peers].sin_port = htons(79);
    peer_sockaddr[num_peers].sin_family = AF_INET;
    num_peers++;
    ...
    // n34 - backend
    memset(&peer_sockaddr[num_peers], 0, sizeof(peer_sockaddr[num_peers]));
    inet_pton(AF_INET, "192.168.11.31", &peer_sockaddr[num_peers].sin_addr);
    peer_sockaddr[num_peers].sin_port = htons(79);
    peer_sockaddr[num_peers].sin_family = AF_INET;
    num_peers++;
    ...
    // n34 - backend
    memset(&peer_sockaddr[num_peers], 0, sizeof(peer_sockaddr[num_peers]));
    inet_pton(AF_INET, "192.168.11.53", &peer_sockaddr[num_peers].sin_addr);
    peer_sockaddr[num_peers].sin_port = htons(79);
    peer_sockaddr[num_peers].sin_family = AF_INET;
    num_peers++;
...
//    ngx_http_handoff_out_loc_conf_t *hocf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_handoff_out_module);
    return NGX_CONF_OK;
}
...
ngx_blocking(c->fd);
        ...
        rc = apply_redirection(migration_info->peer_addr, migration_info->self_addr,
                                    migration_info->peer_port, htons(ntohs(migration_info->self_port) - 1 - 1),
                                    migration_info->peer_addr, my_mac, peer_sockaddr[handoff_out_ctx->peer_to_connect].sin_addr.s_addr, fake_server_mac,
...
//    in_port_t        port;
    ...
    *conn = upstream_conn;
    ...
    //memcpy(rev, r->connection->read, sizeof(ngx_event_t));
    //memcpy(wev, r->connection->write, sizeof(ngx_event_t));
    ...
    memcpy(&sockaddr, &peer_sockaddr[upstream_conn->handoff_out_ctx->peer_to_connect], sizeof(struct sockaddr_in));
...
static ngx_int_t ngx_http_handoff_out_handler(ngx_http_request_t *r) {
    ngx_int_t rc;
    ngx_connection_t *upstream_conn;
    //ngx_http_upstream_conf_t *umcf = ngx_http_get_module_conf(r, ngx_http_upstream_module);
    //ngx_http_handoff_out_loc_conf_t *hocf = ngx_http_get_module_loc_conf(r, ngx_http_handoff_out_module);
    ...
    if (r->connection->handoff_in_ctx == NULL) {
        ...
        handoff_out_ctx->peer_to_connect = my_random(1, num_peers) - 1;
        ...
        rc = get_mac_address(ifname, *addr, client->client_mac);
        assert(rc == 0);
